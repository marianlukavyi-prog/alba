...
export async function generateMetadata({
  ...
  for (const locale of locales) languages[locale] = localePath(locale, '/privacy')
            ...
            {t.sections.map((section) => (
              <section key={section.title} className="flex flex-col gap-2.5">
                ...
                {section.items && section.items.length > 0 ? (
                  <ul className="ml-5 mt-1 flex list-disc flex-col gap-1.5 text-[14px] leading-[1.6] text-[var(--color-brand-soft)] md:text-[15px]">
