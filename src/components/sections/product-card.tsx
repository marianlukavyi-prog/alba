...
export function ProductCard({ product, href, ctaLabel, specLabels }: Props) {
  return (
    ...
    >
      <article className="flex h-full flex-col overflow-hidden bg-[var(--color-surface)] lg:flex-row">
        <div className="relative h-[220px] w-full shrink-0 self-stretch bg-white p-4 md:h-[260px] md:p-6 lg:order-2 lg:h-auto lg:min-h-[300px] lg:flex-1">
          <Image
        ...
        <div className="flex flex-1 flex-col justify-between gap-4 p-4 md:gap-[18px] md:p-[30px] lg:order-1">
          <div className="flex flex-col gap-3 md:gap-[18px]">
            <h3 className="text-[18px] leading-tight font-semibold text-[var(--color-brand)] transition-colors md:text-[20px]">
              {product.name}
            </h3>
          ...
          <RoundArrowButton className="!size-[46px] md:!size-[50px]" />
        </div>
      </article>
    </Link>
  )
